    ...
    <div className="mt-6 sm:mt-8 w-full">
      ...
      {dashData ? (
        <>
          ...
          >
            <h2 className={`text-xl sm:text-3xl font-bold ${darkMode ? 'text-gray-100' : 'text-white'} mb-3 sm:mb-4 flex items-center flex-wrap`}>
              {dashData.symbol}{' '}
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 ${darkMode ? 'text-gray-200' : 'text-white/90'} text-sm sm:text-base`}>
              <p>
                <span className={darkMode ? 'font-medium text-green-400' : 'font-medium text-green-300'}>
                  Open:
                </span>{' '}
                ${dashData['1. open'].toFixed(2)}
              </p>
              <p>
                <span className={darkMode ? 'font-medium text-blue-400' : 'font-medium text-blue-300'}>High:</span>{' '}
                ${dashData['2. high'].toFixed(2)}
              </p>
              <p>
                <span className={darkMode ? 'font-medium text-red-400' : 'font-medium text-red-300'}>Low:</span> $
                {dashData['3. low'].toFixed(2)}
              </p>
              <p>
                <span className={darkMode ? 'font-medium text-purple-400' : 'font-medium text-purple-300'}>
                  Close:
                </span>{' '}
                ${dashData['4. close'].toFixed(2)}
              </p>
              <p className="col-span-1 sm:col-span-2">
                <span className={darkMode ? 'font-medium text-gray-300' : 'font-medium text-gray-200'}>
                  Volume:
                </span>{' '}
